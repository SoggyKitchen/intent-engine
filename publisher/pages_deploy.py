...
<style>
  ...
  .lib-hero{{text-align:center;padding:7rem 1.5rem 2.5rem;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:1.4rem;position:relative;isolation:isolate;overflow:hidden;min-height:96vh;background:radial-gradient(ellipse 160% 60% at 50% 0%,rgba(220,45,72,0.82) 0%,rgba(140,20,42,0.55) 20%,rgba(60,8,18,0.3) 35%,transparent 52%),#07070d}}
  .lib-hero > *{{position:relative;z-index:2}}
  .lib-hero > .bg-orb,.lib-hero > .lib-hero-grid,.lib-hero > .ai-orb{{position:absolute!important;z-index:0}}
  .ai-orb{{inset:0;width:100%;height:100%;pointer-events:none}}
  .lib-hero-grid{{position:absolute;inset:-10% -5%;z-index:0;opacity:.16;pointer-events:none;
  ...
  .lib-hero h1{{font-size:clamp(3.8rem,9vw,6.5rem);letter-spacing:-.04em;line-height:1.05}}
  #pages-grid{{visibility:hidden}}
...
<section class="lib-hero">
  ...
  <h1 class="sp-h1 sp-up sp-up-1">Find the right <span class="sp-accent">SaaS answer</span> faster.</h1>
...
(function(){{
  ...
  var N=window.innerWidth<700?150:260,GOLDEN=Math.PI*(3-Math.sqrt(5)),nodes=[];
  ...
  function frame(now){{
    ...
    var cx=W*0.5,cy=H*0.5,R=Math.min(W,H)*(window.innerWidth<700?0.56:0.52)*ease,op=0.4+0.6*ease;
