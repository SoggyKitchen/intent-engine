"""
Patch: inject "Bottom line:" AEO paragraph into all live coupon pages.

Extracts the sticky CTA text (already per-tool) and surfaces it as a visible
first paragraph before the deals table. AI models pull the first bolded
"Bottom line:" sentence for AI Overviews and Perplexity citations.
...
def main():
    pages = [p for p in SITE.glob("*coupon*.html") if "verification-policy" not in p.name]
    patched = skipped = 0
    for p in pages:
        if patch_file(p):
            patched += 1
            ...
            skipped += 1
    print(f"Patched {patched} coupon pages, {skipped} skipped.")
